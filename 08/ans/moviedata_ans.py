...
class MovieContainer(object):
    ...
    def saveQDataStream(self):
        ...
        try:
            ...
            stream.writeInt32(MovieContainer.FILE_VERSION1)
            ...
            for key, movie in self.__movies:
                ...
                stream.writeQString(movie.acquired.toString(Qt.ISODate))
            ...
            if error is not None:
                return False, error
            self.__dirty = False
            return True, "Saved {} movie records to {}".format(len(self.__movies), QFileInfo(self.__fname).fileName())
    ...
    def loadQDataStream(self):
        ...
        try:
            ...
            elif version > MovieContainer.FILE_VERSION1:
            ...
            if version == MovieContainer.FILE_VERSION:
                while not stream.atEnd():
                    ...
                    acquired = QDate.fromString(stream.readQString(), Qt.ISODate)
            ...
            if error is not None:
                return False, error
            self.__dirty = False
            return True, "Loaded {} movie records from {}".format(len(self.__movies), QFileInfo(self.__fname).fileName())
    ...
    def exportXml(self, fname):
        ...
        try:
            ...
            stream << ("<?xml version='1.0' encoding='{}'?>\n"
                       ...
                       "<MOVIES VERSION='1.0'>\n".format(CODEC))
            for key, movie in self.__movies:
                str = "{}".format(movie.title)
                stream << ("<MOVIE YEAR='{}' MINUTES='{}' "
                           "ACQUIRED='{}' LOCATION='{}'>\n".format(movie.year, movie.minutes, movie.acquired.toString(Qt.ISODate), movie.location)) \
                       << "<TITLE>" << cgi.escape(movie.title) \
                       << "</TITLE>\n<NOTES>"
                       #<< "<TITLE>" << Qt.escape(movie.title) \
                if movie.notes:
                    #stream << "\n" << Qt.escape(encodedNewlines(movie.notes))
                    stream << "\n" << cgi.escape(encodedNewlines(movie.notes))
                stream << "\n</NOTES>\n</MOVIE>\n"
            stream << "</MOVIES>\n"
        except EnvironmentError as e:
            ...
            if error is not None:
                return False, error
            self.__dirty = False
            return True, "Exported {} movie records to {}".format(len(self.__movies), QFileInfo(fname).fileName())
    ...
    def importDOM(self, fname):
        ...
        self.__fname = ""
        self.__dirty = True
        return True, "Imported {} movie records from {}".format(len(self.__movies), QFileInfo(fname).fileName())
    ...
    def readMovieNode(self, element):
        ...
        if len(ymd) != 3:
            raise ValueError("invalid acquired date {}".format(element.attribute("ACQUIRED")))
        acquired = QDate(int(ymd[0]), int(ymd[1]), int(ymd[2]))
        location = element.attribute("LOCATION")
        title = notes = None
        node = element.firstChild()
        while title is None or notes is None or location is None:
            if node.isNull():
        ...
        self.add(Movie(title, year, minutes, acquired, location, decodedNewlines(notes)))
...
class SaxMovieHandler(QXmlDefaultHandler):
    ...
    def startElement(self, namespaceURI, localName, qName, attributes):
        if qName == "MOVIE":
            ...
            if len(ymd) != 3:
                raise ValueError("invalid acquired date {}".format(attributes.value("ACQUIRED")))
            self.acquired = QDate(int(ymd[0]), int(ymd[1]), int(ymd[2]))
            self.location = attributes.value("LOCATION")
    ...
    def endElement(self, namespaceURI, localName, qName):
        if qName == "MOVIE":
            if (self.year is None or self.minutes is None or self.acquired is None or self.location is None or self.title is None or self.notes is None):
                raise ValueError("incomplete movie record")
            self.movies.add(Movie(self.title, self.year, self.minutes, self.acquired, self.location, decodedNewlines(self.notes)))
            self.clear()
        elif qName == "TITLE":
    ...
    def fatalError(self, exception):
        self.error = "parse error at line {} column {}: {}".format(exception.lineNumber(), exception.columnNumber(), exception.message())
        return False