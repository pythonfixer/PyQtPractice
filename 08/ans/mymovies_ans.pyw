...
import addeditmoviedlg_ans
import moviedata_ans
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.movies = moviedata_ans.MovieContainer()
        ...
        fileOpenAction = self.createAction("&Open...", self.fileOpen, QKeySequence.Open, "fileopen", "Open a movie data file")
        fileSaveAction = self.createAction("&Save", self.fileSave, QKeySequence.Save, "filesave", "Save the exists movie data file")
        fileSaveAsAction = self.createAction("Save &As...", self.fileSaveAs, QKeySequence.SaveAs, icon="filesaveas", tip="Save the movie data file as another name")
        ...
        editAddAction = self.createAction("&Add...", self.editAdd, "Ctrl+A", "editadd", "Add a movie data")
        editEditAction = self.createAction("&Edit...", self.editEdit, "Ctrl+E", "editedit", "Edit the movie data")
        editRemoveAction = self.createAction("&Remove...", self.editRemove, "Del", "editdelete", "Remove the movie data")
        helpAboutAction = self.createAction("&About", self.helpAbout, tip="About the application")

        fileMenu = self.menuBar().addMenu("&File")
        self.addActions(fileMenu, (fileNewAction, fileOpenAction, fileSaveAction, fileSaveAsAction, None, fileImportDOMAction, fileImportSAXAction, fileExportXmlAction, None, fileQuitAction))
    ...
    def okToContinue(self):
        if self.movies.isDirty():
            reply = QMessageBox.question(self, "My Movies - Unsaved Changes", "Save unsaved changes?", QMessageBox.Yes | QMessageBox.No | QMessageBox.Cancel)
    ...
    def updateTable(self, current=None):
        ...
        self.table.setHorizontalHeaderLabels(["Title", "Year", "Mins", "Acquired", "Location", "Notes"])
        ...
        for row, movie in enumerate(self.movies):
            ...
            item = QTableWidgetItem(movie.acquired.toString(moviedata_ans.DATEFORMAT))
            item.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter)
    ...
    def fileOpen(self):
        if not self.okToContinue():
            return
        path = (QFileInfo(self.movies.filename()).path() if self.movies.filename() else ".")
    ...
    def fileSaveAs(self):
        ...
        fname = QFileDialog.getSaveFileName(self, "My Movies - Save Movie Data", fname, "My Movies data files ({})".format(self.movies.formats()))
        if fname:
            if "." not in fname:
                fname += ".mqb"
    ...
    def editAdd(self):
        ...
        form = addeditmoviedlg_ans.AddEditMovieDlg(self.movies, None, self)
        if form.exec_():
    ...
    def editEdit(self):
        ...
        if movie is not None:
            ...
            form = addeditmoviedlg_ans.AddEditMovieDlg(self.movies, movie, self)
            if form.exec_():
    ...
    def editRemove(self):
        ...
        if movie is not None:
            ...
            if (QMessageBox.question(self, "My Movies - Delete Movie", "Delete Movie '{}' {}?".format(movie.title, year), QMessageBox.Yes|QMessageBox.No) == QMessageBox.Yes):
