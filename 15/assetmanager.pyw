...
class MainForm(QDialog):
    ...
    def addAsset(self):
        row = (self.assetView.currentIndex().row() if self.assetView.currentIndex().isValid() else 0)
    ...
    def deleteAsset(self):
        ...
        logrecords = 1
        query = QSqlQuery("SELECT COUNT(*) FROM logs WHERE assetid = {}".format(assetid))
        if query.next():
            logrecords = int(query.value(0))
        msg = ("<font color=red>Delete</font><br><b>{}</b>"
               "<br>from room {}".format(record.value(NAME), record.value(ROOM)))
        if logrecords > 1:
        ...
        if (QMessageBox.question(self, "Delete Asset", msg,
            ...
            QSqlDatabase.database().rollback()
            return
        query.exec_("DELETE FROM logs WHERE assetid = {}".format(assetid))
