...
class Form(QDialog):
    ...
    def printViaHtml(self):
        ...
        for statement in self.statements:
            ...
            address = re.escape(statement.address).replace(",", "<br>")
            contact = re.escape(statement.contact)
            ...
            html += ("<p align=right><img src=':/logo.png'></p>"
                     ...
                     "<p>{1}</p><p>Dear {2},</p>"
                     "<p>The balance of your account is {3}.").format(date, address, contact, "$ {:,.2f}".format(balance))
            if balance < 0:
            ...
            for date, amount in statement.transactions:
                ...
                html += ("<tr><td align=right>{0}</td>"
                         ...
                         date.toString(DATE_FORMAT), status, color, "$ {:,.2f}".format(abs(amount))))
    ...
    def printViaQCursor(self):
        ...
        for statement in self.statements:
            ...
            cursor.insertBlock(bodyFormat, bodyCharFormat)
            balance = statement.balance()
            cursor.insertText("The balance of your account is $ {:,.2f}.".format(balance))
            if balance < 0:
    ...
    def printViaQPainter(self):
        ...
        for statement in self.statements:
            ...
            for date, amount in statement.transactions:
                ...
                painter.drawText(QRectF(x + 3, y + 3, DateWidth - 6, h - 6), date.toString(DATE_FORMAT), option)
                x += DateWidth
                painter.drawRect(x, y, CreditWidth, h)
                text = "Credit"
                if amount < 0:
                    text = "Debit"
                painter.drawText(QRectF(x + 3, y + 3, CreditWidth - 6, h - 6), text, option)
                x += CreditWidth
                painter.drawRect(x, y, AmountWidth, h)
                if amount < 0:
                    painter.setPen(Qt.red)
                painter.drawText(QRectF(x + 3, y + 3, AmountWidth - 6, h - 6), "$ {:,.2f}".format(amount), option)
                painter.setPen(Qt.black)
                y += h
            y += serifLineHeight
            x = LeftMargin
